...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        modelViewStack.push();  // Save the current matrix on the
        ...
        earthSphere.render();
        modelViewStack.pop();  // Go to origin
        
        // Planet 1
        modelViewStack.push();                      
        modelViewStack.rotY(0.2f * time);             // Planet 1 orbit rotation
        modelViewStack.translate(-1.0f, 0.0f, 0.0f);  // Planet 1 orbit radius
        modelViewStack.push();

        modelViewStack.rotY(0.8f * time);  // Planet 1 rotation around its axis
        modelViewStack.rotX(
            static_cast<float>(-M_PI_2));  // Orient the poles along Y axis instead of Z
        modelViewStack.scale(0.2f);        // Update the transformation matrix in the shader
        glUniformMatrix4fv(location_MV, 1, GL_FALSE, modelViewStack.getCurrentMatrix());
        // Render the geometry to draw the Planet 1
        ...
        // Planet 2
        modelViewStack.push();
        modelViewStack.rotY(0.2f * time);            // Planet 2 orbit rotation
        modelViewStack.translate(2.0f, 0.0f, 0.0f);  // Planet 2 orbit radius
        modelViewStack.push();

        modelViewStack.rotY(0.8f * time);  // Planet 2 rotation around its axis
        modelViewStack.rotX(
            static_cast<float>(-M_PI_2));  // Orient the poles along Y axis instead of Z
        modelViewStack.scale(0.25f);       // Update the transformation matrix in the shader
        glUniformMatrix4fv(location_MV, 1, GL_FALSE, modelViewStack.getCurrentMatrix());
        // Render the geometry to draw the Planet 1
        glBindTexture(GL_TEXTURE_2D, moonTexture.texID);
        earthSphere.render();
        ...
        modelViewStack.rotY(0.8f * time);  // Earth's rotation around its axis
        modelViewStack.rotX(
            static_cast<float>(-M_PI_2));  // Orient the poles along Y axis instead of Z
        ...
        // Moon
        modelViewStack.translate(1.5f, 0.0f, 0.0f);  // Moon's orbit radius
        modelViewStack.push();                       // Save the matrix before the Earth's rotation

        // modelViewStack.rotY(0.2f * time);                  // Moon's rotation around its axis
        modelViewStack.rotX(static_cast<float>(-M_PI_2));  // Orient the poles along Y axis instead of Z
        modelViewStack.scale(0.4f);        // Update the transformation matrix in the shader
        glUniformMatrix4fv(location_MV, 1, GL_FALSE, modelViewStack.getCurrentMatrix());
        // Render the geometry to draw the earth
        glBindTexture(GL_TEXTURE_2D, moonTexture.texID);
        earthSphere.render();

        modelViewStack.pop();  // Go to Earth
        modelViewStack.pop();  // Go to Sun

        modelViewStack.pop();  // Go to Scene
